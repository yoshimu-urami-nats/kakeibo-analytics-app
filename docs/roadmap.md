...
Djangoデータ編集ページ（transactions）の修正は  
views.py  
...
あとで作りたい機能
- メモの入力機能
- スクロールしたらカラムタイトルだけ見切れないようにする
- 確定済み　False　のみ表示⇔　FalseもTrueも全表示　切替追加
- 選択ファイル名のデータごとにエクスポート/インポート機能  
  　(メンバー・カテゴリ追加済みのものをバックアップcsvからインポート可能にする)
- 確定にしました　の出る位置を明細の上に変更
- いま表示しているファイル名を出すところを明細一覧(○○)　←こうする
- 明細を検索　[]　を明細一覧の上に変更する
- Djangoサマリを月ごとに見れるようにする
- 明細の表示ファイルを選べるようにする

方針メモ  
1️⃣ 月別・カテゴリ別集計（pandas）  
2️⃣ 異常値検出  
3️⃣ 6か月トレンド可視化  
4️⃣ 自動分類精度評価  
5️⃣（余裕あれば）予測
...
初期データの入れ方
admin参考にDBカラムのデータがすべてそろった過去分をCSVで作成してDjangoの管理コマンドでインポートする
（初回限定）